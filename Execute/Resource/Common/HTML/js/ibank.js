...
// 添加底部时间
function appendFooter(parentElementId){
    ...
    const headerDateEle = document.querySelector('.header-date');
    if (headerDateEle) {
        headerDateEle.style.display = 'none';
    }
    
    let deviceid = window?.chrome?.webview
          ? window.chrome.webview.hostObjects.sync.eCAT.GetData?.("core_TerminalID")
          : null;

    let footerBox = document.createElement('div');
    footerBox.setAttribute('class', 'footer-box');
    ...
    deviceImg.setAttribute('src', '../../images/img_2.4/Device.png');

    let deviceIdText = document.createElement('span');
    deviceIdText.setAttribute('ids', 'ids_Common_DeviceID');
    deviceIdText.textContent = 'Device ID:';

    let deviceIdSpan = document.createElement('span');
    deviceIdSpan.setAttribute('id', 'deviceid-span');

    footerItem1.appendChild(deviceImg);
    footerItem1.appendChild(deviceIdText);
    footerItem1.appendChild(deviceIdSpan);

    let footerItem2 = document.createElement('div');
    footerItem2.setAttribute('class', 'footer-item');

    let timeImg = document.createElement('img');
    timeImg.setAttribute('src', '../../images/img_2.4/Time.png');

    let timeBox = document.createElement('span');
    timeBox.setAttribute('class', 'time-box');

    let dateSpan = document.createElement('span');
    dateSpan.setAttribute('id', 'date-b');
    dateSpan.textContent = '2024-12-18';

    let timeSpan = document.createElement('span');
    timeSpan.setAttribute('id', 'time-b');
    timeSpan.textContent = '00:00:00';

    timeBox.appendChild(dateSpan);
    timeBox.appendChild(timeSpan);

    footerItem2.appendChild(timeImg);
    footerItem2.appendChild(timeBox);

    footerBox.appendChild(footerItem1);
    footerBox.appendChild(footerItem2);

    parentElement.appendChild(footerBox);

    const deviceidSpan = document.querySelector('#deviceid-span');
    if (deviceidSpan) {
