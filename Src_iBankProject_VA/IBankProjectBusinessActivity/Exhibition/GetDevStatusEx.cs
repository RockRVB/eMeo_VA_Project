...
{
    [GrgActivity("{F00B8C20-2465-49EF-9C3F-D9651BCA0CA8}",
                  Name = "GetDevStatusEx",
                  NodeNameOfConfiguration = "GetDevStatusEx",
                  Author = "  ")]
    public class GetDevStatusEx : BusinessActivityGetDevStatus
    {
        #region constructor
        protected GetDevStatusEx()
        {
        ...
        private bool CheckDevStatus(string devName)
        {
            ...
            else if (devName.Equals("FeedChannel"))
            {
                if (null != VTMContext.FeedChannel)
                {
                    VTMContext.FeedChannel.UpdateStatus();

                    if (!CheckStatus(VTMContext.FeedChannel))
                    {
                        SwitchUIState(VTMContext.MainUI, DataDictionary.s_DefaultUIState, -int.MaxValue);

                        VTMContext.LogJournalKey("IDS_FeedChannelDevFailure", TextCategory.s_journal, LogSymbol.DeviceFailure);

                        DevResult devResult = VTMContext.FeedChannel.Reset();
                        if (devResult.IsFailure)
                        {
                            VTMContext.LogJournalKey("IDS_ResetFeedChannelDevFailure", TextCategory.s_journal, LogSymbol.DeviceFailure);
                            return false;
                        }
                        else
                        {
                            ...
                            {
                                VTMContext.LogJournalKey("IDS_FeedChannelResetSuccess", TextCategory.s_journal, LogSymbol.DeviceFailure);
                                return true;
                            }
                        }
                    }
                    ...
                    {
                        VTMContext.FeedChannel.UpdateStatus();
                        GrgCmdFeedChannelStatusInfo feedStatus;
                        VTMContext.FeedChannel.GetStatusInfo(out feedStatus, m_devTimeout);

                        if (null != feedStatus && (HardwareState.Online == feedStatus.DeviceState || HardwareState.Busy == feedStatus.DeviceState) && feedStatus.MediaState == MediaState.NotPresent)
                        {
                            return true;
                        }
                        else
            ...
            else if(devName.Equals("PassPort")) //护照设备复位 add by xjyong
            {
                if (null != VTMContext.PassPortDevice)
                {
                    if (!CheckStatus(VTMContext.PassPortDevice, "GetStatusInfo"))
                    {
                        SwitchUIState(VTMContext.MainUI, DataDictionary.s_DefaultUIState, -int.MaxValue);
                        VTMContext.LogJournalKey("IDS_ScanPassportFailure", TextCategory.s_journal, LogSymbol.DeviceFailure);
                        DevResult devResult = VTMContext.PassPortDevice.Reset();
                        Log.Action.LogDebug("PassPort Reset");
                        if (devResult.IsFailure)
                        {
                            VTMContext.LogJournalKey("IDS_ResetPassportFailure", TextCategory.s_journal, LogSymbol.DeviceFailure);
                            return false;
                        }
                        else
                        {
                            ...
                            {
                                VTMContext.LogJournalKey("IDS_PassportResetSuccess", TextCategory.s_journal, LogSymbol.DeviceFailure);
                                return true;
                            }
                        }
                    }
                ...
                {
                    VTMContext.LogJournalKey("IDS_PassportNoDevFailure", TextCategory.s_journal, LogSymbol.DeviceFailure);
                    return false;
                }
            }
            else if (devName.Equals("FingerVein"))
            {
                if (null != VTMContext.FingerVeinPatch)
                {
                    VTMContext.FingerVeinPatch.UpdateStatus();

                    if (!CheckStatus(VTMContext.FingerVeinPatch))
                    {
                        SwitchUIState(VTMContext.MainUI, DataDictionary.s_DefaultUIState, -int.MaxValue);

                        VTMContext.LogJournalKey("IDS_FingerVeinFailure", TextCategory.s_journal, LogSymbol.DeviceFailure);

                        DevResult devResult = VTMContext.FingerVeinPatch.Reset();
                        if (devResult.IsFailure)
                        {
                            VTMContext.LogJournalKey("IDS_FingerVeinFailure", TextCategory.s_journal, LogSymbol.DeviceFailure);
                            return false;
                        ...
                        else
                        {
                            ...
                            {
                                VTMContext.LogJournalKey("IDS_FingerVeinResetSuccess", TextCategory.s_journal, LogSymbol.DeviceFailure);
                                return true;
                            }
                        }
                    }
                ...
                {
                    VTMContext.LogJournalKey("IDS_FingerVeinNoDevFailure", TextCategory.s_journal, LogSymbol.DeviceFailure);
                    return false;
                }
            }
            else
        ...
        {
            GrgCmdDispenserStatus dispenserStatue = devInfo as GrgCmdDispenserStatus;

            if (dispenserStatue != null && !dispenserStatue.DispenserStatus.Equals("OK"))
            {
                Log.Action.LogDebug("Dispenser Status is not OK!");
                return false;
            }
            ...
            if (null != carUnitList && carUnitList.Count > 0)
            {
                ...
                {
                    if(cardUnit.Type == emCRDType.RetainBin && cardUnit.Status.Equals("FULL"))
                    {
                        isAllCardEmpty = false;
                        break;
                    }
                    else if (cardUnit.Type == emCRDType.SupplyBin && cardUnit.Count > 0)
                    {
                        isAllCardEmpty = true;
                    }
                }
     ...
     null);
                    ...
                    {
                        if (devType.Equals("UkeyDispenser") || devType.Equals("UkeyRightDispenser"))
                        {
                            return CheckUkeyDevStateEX();
                        }
                        else
                        {
                            return true;
        ...
        /// 检查Ukey箱状态是否异常
        /// </summary>
        /// <returns>true无异常 false异常</returns>
        private bool CheckUkeyDevStateEX()
        {
            GrgCmdDispenserStatus ukeyStatue = devInfo as GrgCmdDispenserStatus;

            if (ukeyStatue.TransportStatus.Equals("OK") && ukeyStatue.ShutterStatus.Equals(DoorStatus.Closed)) 
            {
                return true;
            }
            else
